...
export const CODING_NOTES: Note[] = [
  ...
  },
  {
    ...
    severe: true,
    issue: "Together HAS now published a platform, at be-yahad.org.il/en/plans, and this column has not been re-coded against it. That is a materially different admission from the one this note used to make, and a worse one: coding a column from press statements is defensible while no document exists and indefensible once one does. Two sections have since been read in — education and ageing — and B3 is the only cell they settle, the first in this column sourced to the party's own document. Every other cell is still derived from Yesh Atid's prior platform plus Bennett's public statements, and the two diverge — on religion-state and settlements, and, as of the August reporting on Bennett's security thinking, on the Palestinian state and the Palestinian Authority as well. That is the whole security core of the bank, so the divergence can no longer be described as confined to two blocks. The column is nonetheless coded on every item and therefore ranked in full, which is the exposure: the riskiest column in the bank is not the one the coverage floor suppresses. Reading the rest of the platform in is the highest-value single task outstanding on the bank, ahead of the sixteen flagged Joint List cells, because this column is ranked and that one is a merge artefact. The sections that matter most are the ones the education plan does not touch: security, Gaza and the Palestinian Authority, where A1, A6 and A16 remain the exposure.",
  },
  {
    items: "All Yashar rows",
    issue: "Eisenkot's positioning is deliberately broad, and N was doing duty for that broadness in cells where the party had simply said nothing. A5, A6, A9 and A10 are now '-' rather than N, which is what this note had recommended and had not acted on; coverage reads 92% and is honest rather than 100% and flattering. A1 is no longer one of them: asked directly in August about the two-state solution, he called anyone still thinking about it after 7 October delusional, and the cell moved from N to D. The same interview confirms three cells that were already coded — A7, A8 and C9 — since he called E1 a mistake costing Israel internationally, described settler violence as endangering the settlement movement, and named the transfer of the police to Ben Gvir and COGAT to Smotrich as the reason the IDF lost control of the West Bank. It is the first substantial evidence this column has had, and the direction of travel is that fewer of its N codings will survive contact with an interview, not more.",
  },
  {
    items: "N versus '-' · the distinction the scoring engine depends on",
    severe: true,
    issue: "N and '-' are not interchangeable and were being used as though they were. N maps to direction zero, which means a respondent answering neutral scores full agreement with it, a respondent answering either way scores half, and the cell counts toward the party's coverage. '-' leaves the denominator entirely. So an N standing in for an unknown does not merely fail to inform — it manufactures a centrist position and then rewards a centrist respondent for matching it. The Yashar note had said for months that A5, A6, A9 and A10 were N for want of evidence rather than because the party is equivocal; those four are now '-', and Yashar's coverage reads 92% instead of a false 100%. Together loses A1, A6 and D2 on the parallel argument below. What has not been done is the sweep: every remaining N in every column should be made to answer whether there is affirmative evidence the party is genuinely equivocal, and any that cannot should become '-'. That audit is worth more than any new item, and it is the first task of the September revision.",
  },
  {
    items: "A1, A6, D2 (Together) · divergence is not compromise",
    severe: true,
    issue: "Coding a merged list N where its components disagree asserts something specific: that the ballot entity has adopted a compromise. Where Bennett and Lapid have opposite public records and the list has published nothing resolving them, the true statement is that nobody knows what Together's position is, and that is '-'. A1 and A6 were coded D from Bennett alone and D2 was coded N under the merge rule; all three are now '-'. The effect is smaller than the argument implies and the difference is worth stating: it moves three cells, not forty, because only three are documented divergences. The rest of the column is inferred from Yesh Atid's prior platform where Bennett agrees or is silent, which is weak sourcing but not unresolved disagreement, and the rule does not reach it. Coverage falls from 100% to 94%. The published platform is what refills these, one section at a time.",
  },
  {
    items: "A3, A4 (Likud)",
  ...
  {
    items: "A5 (all columns) · swapped to the durable wording",
    severe: true,
    issue: "Replaced a hostages item made obsolete when the last remains were returned in January, and has now been replaced in turn. The live wording asked about accepting the Board of Peace's phased disarmament plan and was the most perishable item in the bank by design: Hamas accepted the roadmap framework, Israel rejected the present sequencing, and force deployment moved into negotiation, so the row was about to describe a proposal nobody was still arguing about in those terms. A5_VARIANT is now durable — Israeli forces should withdraw from Gaza only once Hamas has completely disarmed — which measures the same cleavage, sequencing, without depending on one plan surviving under one name. Swapped before the live wording became false rather than after, and before the validation round rather than during it, which is the only cheap moment to do it."
  },
  {
    items: "Item selection · the ruler and the thing measured",
    severe: true,
    issue: "A standing methodological limit, recorded because it cannot be fully fixed from the inside. Several items exist, or sit where they do, because of what they did to party placement. A13 was added because The Democrats, Ra'am and the Joint List had collapsed onto one security coordinate; D7 was added to break another collapse; A14 sits on security and B14 on religion partly because the other placement would have put a party somewhere its politics says it does not belong. That makes party placement both an input to the instrument and an output of it, and the risk is circular: expect parties to differ, add items that make them differ, then report that they differ. The defence is not the same in every case. For A14 and B14 the argument is about which construct the item measures — Temple Mount prayer is a sovereignty assertion rather than a measure of religiosity, and the evidence that the other placement was wrong is that it scored the strictest rabbinic position as secular. That is abductive, not circular. For A13 and D7 the honest description is that a collapse prompted the search and the construct argument was made afterwards. A13 is the one to watch: condemning attacks on soldiers without qualification does distinguish the Arab parties, but a respondent can oppose the occupation, condemn the attacks and remain thoroughly dovish, so whether it belongs on the security axis is genuinely open. The test is item-axis correlation against real respondent answers, which is one of the things the collection endpoint exists to make possible and which no further argument substitutes for. Until that data exists, discrimination is a secondary criterion for keeping an item and is not evidence that it measures what its block claims.",
  },
  {
  ...
  {
    items: "B3 (Together) · the first cell read off the platform",
    issue: "Confirmed from the party's own education plan rather than inferred, which no other cell in this column can say. Its fourth plank — one nation, one state education system — ends state funding for partisan systems, institutions exempt from regulation and separatist sectoral schools, and requires 60 percent of the curriculum to be the national core. A was already the coding and the platform is stronger than the coding: B3 asks whether funding should be conditional on teaching the core curriculum, and Together's position is a mandated proportion, not a condition. Worth recording for the September revision that the item names yeshivot specifically while the policy actually on offer covers every non-state-supervised stream, haredi and Arab alike. That distinction changes nothing for Together, which is A either way, but it is the sort of narrowing that makes an item easier to answer and less accurate about what is being proposed.",
  },
  {
    items: "E1 (Together) · the cell the platform pushes hardest on",
    issue: "Left at N, and it is the least comfortable N in the column. Together's ageing plan is a large and specific welfare expansion: pensions indexed to wages rather than prices, differential increases for some 350,000 seniors with the poorest pensions doubled, wider property-tax and electricity discounts, higher nursing benefits, funded dementia and loneliness programmes, and an indexed Holocaust survivors' allowance. Read as willingness to spend on health and welfare that is plainly A. The item is not only about willingness to spend. It asks about spending more even where that requires higher taxes, and the same platform runs the other way on the funding side — market opening to cut food prices, further privatisation — and never proposes a tax rise. A party promising both is what N is for, and it is what Blue & White is coded for the same reason. Recorded for September alongside the B3 observation: the conditional clause is the whole difficulty. An item that bundles a spending preference with a tax preference cannot code a party that holds one and not the other, and this platform is the clearest case in the bank of a party that does.",
  },
  {
    items: "D2 (Together)",
  ...
  {
    items: "A16 (all columns) · rewritten after it was found wrong",
    severe: true,
    issue: "The first wording asked whether security control should pass to a force of Arab and Muslim states, and it was wrong twice. The stabilisation force actually under discussion is not Arab and Muslim — Albania, Indonesia, Kazakhstan, Kosovo, Uganda and Burundi are among those named — so the phrase attached an identity cue to a proposal that does not carry one, which is the §5 failure this bank exists to catch. And the force does not receive security control: the roadmap has it monitoring the ceasefire, supporting demilitarisation and training police, while Palestinian civilian police do the policing. The row compressed three actors into two and would have produced coding errors from sources that were themselves correct. The rewrite names the institutional choice instead — Palestinian policing under international backing against permanent IDF control — and survives changes in which countries contribute troops. What must not survive is the original justification. A16 was defended partly as the row that finally gave Together a security signal and partly on discrimination, at 72% of party pairs. Under the corrected wording Together is '-', because Bennett's stated mechanism is Egyptian troops rather than Palestinian police and the list has resolved nothing, and discrimination has fallen to 57% because five columns now honestly say nothing. The item stands on the one ground that was always the real one: nothing else in the bank asks who polices Gaza."
  },
  {
    items: "A16 (Likud, and the columns that went silent)",
    issue: "Likud is the only matrix column on the disagreeing side coded from statement rather than inference: its insistence on permanent Israeli freedom of action in Gaza is stated and repeated, so D is safe. Yisrael Beiteinu was D in the first version, on Liberman's standing argument that international forces do not fight — which he has made about UNIFIL for two decades and has not made about this structure. Under the rule that an unevidenced position is '-' rather than a guess it is now '-', and Shas and United Torah Judaism went the same way: neither has addressed who polices Gaza, and neither is plausibly equivocal about it rather than simply unasked."
  },
  {
...
export const INSTRUMENT_NOTES: Note[] = [
  ...
  {
    items: "A7 · why there is no E1 item",
    issue: "Considered in August after Smotrich's Negev and Galilee plan and rejected on the numbers. E1 is the most consequential single settlement decision live in this cycle, and it does mark a real cleavage — Likud has frozen it under American pressure where Religious Zionism and Otzma Yehudit push it — so the case for asking about it directly is not weak. Coded and measured, though, it fails outright. The first pass put Likud at N on its record of freezing E1 under American pressure, which was already out of date: the government approved the project and has since tendered over a thousand homes, so Likud is A. With that correction the row is character-for-character identical to A3 across all thirteen columns — the exact test under which six items were retired. Asked instead as construction proceeding over American objection it reaches 67% and sits two cells from four existing items, still making no cut nothing else makes. Twenty live items already separate Likud from both parties to its right. There is also a §5 problem that would bite first: E1 cannot be identified in under thirty words without taking a side, since describing it by what it would sever is the opponents' framing and a supporter would not accept it. B12 works as a concrete item because a hospital ward at Passover needs no explanation. This one does.",
  },
  {
    items: "Security axis (Yashar, Blue & White)",
    issue: "The two now sit on the same security coordinate, at -13.3. The collision is new and was caused by moving Yashar to D on A1; before that its silence there held it slightly apart. It is recorded rather than corrected, because the reading is defensible: two former chiefs of staff running anti-Netanyahu lists, both hawkish on the conduct of the war and both opposed to annexation, are genuinely close on this axis. Unlike the older Democrats-Ra'am collision they are not far apart on the other grid axis either — 36 points on religion against that pair's 167 — so they plot near each other rather than merely sharing a number. If the September platforms separate them, this resolves itself; if they do not, the honest description is that the instrument cannot tell these two apart on security, and it should say so rather than manufacture a difference.",
  },
  {
    items: "A8 (Yisrael Beiteinu) · confirmed, and the item is narrower than the politics",
    issue: "Liberman says Israel should restore administrative detention against violent Jewish extremists in the West Bank. A8 is coded A for this column and stays A, now sourced to a statement rather than inherited from the party's general posture — the first substantial evidence about Yisrael Beiteinu this bank has recorded, in a column carrying fifteen N codings that the sweep still has to examine. It moves none of them: a position on enforcement against settler violence says nothing about whether settlement construction should expand, which is A7 and stays N. The observation worth keeping is that the measure is sharper than the item. A8 asks about prosecution; administrative detention is detention without charge, and a respondent can support prosecuting settler violence while refusing to extend to Jewish citizens a tool used overwhelmingly against Palestinians. That is a real position and the bank cannot express it. Note also what makes the proposal politically pointed, which the coding cannot carry either: the argument is symmetry, and the objection to it comes from both directions at once.",
  },
  {
    items: "The September revision has a wording agenda, not only a coding one",
    severe: true,
    issue: "Three independent observations have now accumulated from three unrelated sources, and together they say the revision should re-read statements as well as re-read parties. B3 asks whether state funding for yeshivot should depend on teaching the core curriculum, while the policy actually on offer — Together's, and the Haredi Public Party's — covers every non-state-supervised stream, Arab schools included; the item is narrower than the debate. E1 bundles a spending preference with a tax preference, and cannot code a party that holds one without the other, which is precisely what Together's ageing platform describes. A8 asks about prosecution where the live proposal is administrative detention, a harsher measure that draws objection from people who support the prosecution. In each case the coding is defensible and the item is the thing constraining what can be recorded. None of the three was found by re-reading the bank; each surfaced when a party said something the bank could not quite hold, which is an argument for doing the wording pass against the filed platforms rather than in the abstract.",
  },
  {
