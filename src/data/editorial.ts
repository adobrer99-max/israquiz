...
export const CODING_NOTES: Note[] = [
  {
    items: "A party may be missing from this bank entirely",
    severe: true,
    issue: "Lists closed on 8 September and reporting on the filings names a party this registry does not have: The Reservists, led by Yoaz Hendel, submitted alongside Yashar, B'Yachad, The Democrats, Ra'am, Otzma Yehudit and People of Israel. It has no column here, which is a worse failure than any miscoded cell. A wrong coding gives a voter a number that is off; a missing column gives them a ranking that silently omits something on their ballot, with no marker anywhere on the page to say so. It is recorded rather than added because the only source available here is a summary of news reporting, and building a fifty-cell column on that would be exactly the sourcing this project spent August removing. What is needed is the filed slate and whatever platform exists. Until then the honest statement is that this instrument does not cover the full ballot, and that is a stronger caveat than anything currently on the results page. The same check has not been completed for the rest of the field: Likud had reportedly not filed at press time, and every column here should be confirmed against the Central Elections Committee list rather than against the expectation that nothing else moved.",
  },
  {
